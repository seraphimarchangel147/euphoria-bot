...
def test_hook_tuples_are_adapted_so_distance_is_not_read_as_touched():
    """GridBoard.on_quote_label emits (m, d, h, touched).
    observe_many tuples are (m, touched, d, h). Without an adapter,
    distance=0 is read as touched=False and the label is lost.
    SYNTHETIC. Not a live tape result.
    """
    from src.control.room import ControlRoom

    room = ControlRoom(
        enable_oracle=False, enable_wallet=False, dry_run=True, learn=False,
    )
    # A miss at 1.13 would look like a hit if d=0 were taken as touched.
    room._ingest_quote_labels([(1.13, 0, 30.0, False)])
    assert room.cell_edge.evidence(1.13) == (0, 1, 0.0)
    room._ingest_quote_labels([(1.13, 0, 30.0, True)])
    assert room.cell_edge.evidence(1.13) == (1, 2, 0.5)
    room.close()


def test_the_room_attaches_the_quote_label_hook(tmp_path):
    """Wire only. The running 8765 will not see this until the next start."""
    from src.control.room import ControlRoom
    ...
    room = ControlRoom(
        ...
        player_path=tmp_path / "pl.json", player_calibration_path=tmp_path / "pc.json",
        cell_edge_path=tmp_path / "ce.json",
    )
    assert room.board.on_quote_label.__func__ is room._ingest_quote_labels.__func__
    room._ingest_quote_labels([(1.04, 0, 8.0, True)])
    room.save_cell_edge()
    raw = (tmp_path / "ce.json").read_text()
    assert "104" in raw
    room.close()


def test_a_throwing_quote_consumer_cannot_stop_grading():
    """gridboard already swallows on_quote_label exceptions. Do not weaken it."""
    import inspect
    from src.analytics import gridboard as gb

    src = inspect.getsource(gb.GridBoard.settle)
    assert "on_quote_label" in src
    assert "except Exception" in src
    assert "a consumer must never stop the grading" in src

...
def _room(tmp_path: Path, **kw) -> ControlRoom:
    ...
    # Never let a test read or write the operator's real learned calibration.
    kw.setdefault("calibration_path", tmp_path / "calibration.json")
...
def test_status_always_includes_think_payload(tmp_path):
    room = _room(tmp_path)
    status = room.status()
    think = status["think"]
    assert think is not None
    assert "grade" in think
    assert "tape" in think
    assert "pick" in think
    assert "stand_aside" in think
    room.close()
...
def test_http_start_stop_and_think(tmp_path):
    ...
    try:
        ...
        assert 'id="teach"' in dash.text
        assert 'id="sitPill"' in dash.text
        assert 'id="call"' in dash.text
        assert "paintCall" in dash.text
        assert "view.lesson" in dash.text
        assert "applySnapshot(status)" in dash.text
        assert "applySnapshot(JSON.parse(ev.data))" in dash.text
        assert "book.hit_rate" in dash.text
        assert "sessionPct == null" in dash.text
        assert "tape.last_closed" in dash.text
        assert "hasGhost" in dash.text
        assert "rawPick" in dash.text
        assert 'id="quoteStrip"' in dash.text
        assert 'id="edgeLine"' in dash.text
        assert "paintQuote" in dash.text
        assert "player_edge" in dash.text
        assert "pinMult" in dash.text
    finally:
        httpd.shutdown()
        httpd.server_close()
        room.close()

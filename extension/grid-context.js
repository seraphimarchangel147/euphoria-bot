...
(function attachGridContext(root, factory) {
  ...
  // The forecaster prices any cell it is handed, so hand it a real grid rather
  // than the two squares next to the money. Six columns is ~30s ahead; four
  // rows either side covers the diagonals where the fat multipliers sit.
  const FORWARD_COLUMNS = 6;
  const ROW_RADIUS = 4;
  ...
  function buildGridContext(options) {
    ...
    // Prefer a feed found by capability; fall back to the chart wrapper.
    const feed = opts.quotesFeed || (chart && chart.quotesFeed);

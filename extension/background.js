...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  (async () => {
    ...
    if (msg.type === "session") {
      ...
      if (msg.wallet) extra.wallet = msg.wallet;
      if (msg.frames) extra.frames = msg.frames;
    ...
    if (msg.type === "player-event") {
      ...
      /* Fan out to both sinks. The bridge keeps its existing feed; the control
         room needs these to learn from real money rather than only from the
         free touch labels it grades for itself. `post` swallows failures, so a
         dead control room cannot break the bridge path or this response. */
      const [bridged, roomed] = await Promise.all([
        event && typeof bridgeFetch === "function"
          ? bridgeFetch("/euphoria/player-event", { method: "POST", body: event })
          : Promise.resolve(null),
        event ? post("/player-event", event) : Promise.resolve(null),
      ]);
      sendResponse(event
        ? { ok: true, bridge: bridged, room: roomed }
        : { ok: false, error: "invalid player event" });
      return;
    }
    if (msg.type === "status") {

...
Ingest matches live VolRegime: None, non-float, <=0, NaN, and ±inf
are dropped. A NaN in the window would poison ``statistics.quantiles``.
"""
...
class RollingVolRegime:
    ...
    def observe(self, value: Any, *, now: float) -> None:
        """Store ``(now, value)`` if it is a finite positive house vol.

        Same ingest as live VolRegime: TypeError/ValueError, <=0, NaN,
        and ±inf are dropped so they cannot enter the window.
        ``now`` is the only clock.
        """
        try:
            vol = float(value)
        ...
        if not (vol > 0) or vol != vol or vol in (float("inf"), float("-inf")):
            return
        self._samples.append((float(now), vol))
